...
module.exports = function (staleness) {
...
function createEntryUpdateStream (all, auth, staleness, latest, cb) {
  ...
  if (latest && (Date.now() - latest < (staleness * 1000))) {
    ...
    log.info('all-package-metadata', 'Local data up to date, skipping update')
    return cb(null)
  } else if (latest === 0) {
    log.warn('', 'Building the local index for the first time, please be patient')
    log.silly('all-package-metadata', 'No cached data: requesting full metadata db')
  } else {
